...
local_path = os.path.join(parent_dir, "data_files/local_defect_top.dat")
global_path = os.path.join(parent_dir, "data_files/global_defect_top.dat")
freq_path = os.path.join(parent_dir, "data_files/defect_freq_area_top.dat")
 
# select image input directories:
lipid_path = os.path.join(parent_dir, "top/top_lipid")
prot_path = os.path.join(parent_dir, "top/top_prot")
...
################################
	...
	if i.endswith(".bmp"):
		...
		print("Number of non-zero elements in pbc_select:", np.count_nonzero(pbc_select))
		# select the contour corresponding to the unit cell box
		#(_, box_c, _) = cv2.findContours(pbc_select, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
		(box_c, _) = cv2.findContours(pbc_select, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
			# draw the contour on the original mask and fill it in
		print(box_c)
		if len(box_c) > 0:
...
###################################
		...
		# find all contours
		#(_,cnts_all, _) = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
		(cnts_all, _) = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
		...
		#(_,cnts_good, _) = cv2.findContours(good_defects, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
		(cnts_good, _) = cv2.findContours(good_defects, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
